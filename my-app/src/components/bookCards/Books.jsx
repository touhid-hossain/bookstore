...
import { useDeleteBooksMutation } from "../../features/api/apiSlice";
import Error from "../loaders/Error";

const Books = ({ book }) => {
  const { id, name, author, thumbnail, price, rating, featured } = book;
  const [deleteBooks, { isError, isLoading }] = useDeleteBooksMutation();
  ...
  return (
    <div className="book-card" key={id}>
      ...
      {!isLoading && isError && (
        <Error message="There was an error detected deleting the video!!" />
      )}
    </div>
  );
};
